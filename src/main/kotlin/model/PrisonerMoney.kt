...
class PrisonerMoney private constructor() {
  companion object : HMPPSSoftwareSystem {
    ...
    // lateinit var natwest: Enterprise
    lateinit var natwestFtp: SoftwareSystem
    lateinit var govpay: SoftwareSystem
    lateinit var worldpay: SoftwareSystem
    lateinit var mtpBankAccount: SoftwareSystem
    lateinit var prisonBankAccount: SoftwareSystem
    ...
    override fun defineModelEntities(model: Model) {
      ...
      sendMoneyService = system.addContainer(
        "Prisoner Money Send-Money Application",
      ...
      bankAdminService = system.addContainer(
        "Prisoner Money Bank Admin Application",
        "SSCL staff facing site for Prisoner Money", "Django App"
      ).apply {
      ...
      cashbookService = system.addContainer(
        "Prisoner Money Cashbook Application",
        "Business hub staff facing site for Prisoner Money suite of apps", "Django App"
      ).apply {
      ...
      nomsOpsService = system.addContainer(
        "Prisoner Money noms-ops Application",
        "Web application to allow for management of functionality related to the delayed-capture of credits", "Django App"
      ).apply {
      ...
      worldpay = model.addSoftwareSystem("worldpay", "External payment service")
        .apply {
          OutsideHMPPS.addTo(this)
        }
      ...
      mtpBankAccount = model.addSoftwareSystem("Money to Prisoners bank account", "Business bank account provided by Natwest")
        .apply {
          OutsideHMPPS.addTo(this)
        }
      prisonBankAccount = model.addSoftwareSystem("Prison bank account", "Bank account of a given (public/private) prison")
        .apply {
          OutsideHMPPS.addTo(this)
        }
      ...
      transactionUploaderCronJob = system.addContainer(
        "Prisoner Money",
        "Cron job that fetches the previous days bank transaction records from natwest FTP server", "Django App"
      ).apply {
      ...
      sendMoneyPublicUser = model.addPerson("Send Money User", "Any member of the public with a debit card, and the necessary information for a particular prisoner (i.e a prisoner_id and date of birth)").apply {
        OutsideHMPPS.addTo(this)
      }
    ...
    override fun defineRelationships() {
      sendMoneyPublicUser.uses(sendMoneyService, "Visits send money public site to transfer money from their debit card to the Private account of the specified prisoner for them to spend the money at the prison Canteen (after the money is transferred to their Spends account)", "HTTPS")
      fiuUser.uses(nomsOpsService, "Views payments flagged as suspicious and either permits it or refunds it back to the sender without crediting that prisoners account", "HTTPS")
      ...
      govpay.uses(worldpay, "to trigger a transfer from a given debit card", "HTTPS")
      ...
      // sendMoneyPublicUser.uses(govpay, "to transfer from their debit card to a given prisoner", "HTTPS")
      worldpay.uses(mtpBankAccount, "to deposit monies paid to it by members of the public", "Bank transaction (daily, as a batch)")
      ssclUser.uses(mtpBankAccount, "to transfer money to the prison", "Bank transaction (daily, as a batch)")
      mtpBankAccount.uses(prisonBankAccount, "to transfer money into", "Bank transaction (daily, as a batch)")
      mtpBankAccount.uses(prisonBankAccount, "to transfer money into", "Bank transaction (daily, as a batch)")
      // mtpBankAccount.uses(sendMoneyPublicUser, "to transfer money to from prisoner disbursement", "Bank transaction (daily, as a batch)")


        // natwestFtp.setEnterprise(natwest)
        // mtpBankAccount.setEnterprise(natwest)
    }
